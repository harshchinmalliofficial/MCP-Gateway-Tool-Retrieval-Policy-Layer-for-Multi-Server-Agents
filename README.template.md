...
**Headline from the measured run ({{MODEL_NOTE}}):**

- Caching made tool fetching **{{FETCH_SPEEDUP}}× faster** and left accuracy
  unchanged ({{ACC_DELTA_AB}} pp) — it's a speed fix, not an accuracy fix.
- Retrieval sent the LLM **{{TOK_RATIO}}× fewer tokens** ({{TOK_B}} → {{TOK_C}})
  and cut end-to-end latency **{{LAT_CUT_PCT}}%** ({{LAT_B}} → {{LAT_C}}) for a
  {{RETR_MS}} ms retrieval step. Accuracy went {{ACC_B}} → {{ACC_C}}: this model
...
- The latency lines **cross at {{CROSSOVER}} tools**. Below that the gateway
...
| # | Claim | Verdict |
|---|-------|---------|
| 1 | Caching speeds up tool **fetching** but does **not** improve tool-selection accuracy | {{VERDICT_1}} |
| 2 | FAISS retrieval improves **accuracy** AND cuts **tokens** AND lowers **end-to-end latency**, despite adding a retrieval step | {{VERDICT_2}} |
| 3 | There is a **crossover** (~20–40 tools): below it the gateway isn't worth it, above it it clearly wins | {{VERDICT_3}} |
...
_Run metadata: {{RUN_META}}_

### Why Claim 2 came back partial (intellectual honesty)

Claim 2 has three parts. **Tokens** and **latency** held decisively: retrieval
sent **{{TOK_RATIO}}× fewer tokens** ({{TOK_B}} → {{TOK_C}}) and cut end-to-end
latency **{{LAT_CUT_PCT}}%** ({{LAT_B}} → {{LAT_C}}), for a {{RETR_MS}} ms
retrieval step.

The **accuracy** part didn't — because there was nothing to fix. On this clean,
well-separated 157-tool catalogue, `{{PROVIDER}}` picked the correct tool
**{{ACC_B}} of the time with the entire list in context** (and 100% at every
...
{{ACC_C}} — a single query whose target ranked 6th in FAISS and fell just outside
the top-5.
...
| What it changes | *Fetch latency* ↓ (here {{FETCH_SPEEDUP}}×) | *Tokens* ↓ ({{TOK_RATIO}}×), *latency* ↓ ({{LAT_CUT_PCT}}%), *model confusion* ↓ (when the model isn't already at ceiling) |
...
| Cost it adds | ~0 (dict lookup + TTL check) | one query embedding + a FAISS search + hybrid re-rank ({{RETR_MS}} ms here) |
...
- **Under ~{{CROSSOVER}} tools.** The crossover experiment shows the retrieval
...
- **Provider used for the numbers above: `{{PROVIDER}}`.** Groq is the configured
